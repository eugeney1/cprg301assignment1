...
export default class pixelit {
  ...
  pixelate() {
    ...
    if (this.drawfrom.naturalWidth === 0 || this.drawfrom.naturalHeight === 0) {
      console.error("Source image not loaded");
      return this;
    }

    // Set safe dimensions
    this.drawto.width = Math.max(1, this.drawfrom.naturalWidth);
    this.drawto.height = Math.max(1, this.drawfrom.naturalHeight);

    // Calculate scaled dimensions with validation
    let scaledW = Math.max(1, this.drawto.width * this.scale);
    let scaledH = Math.max(1, this.drawto.height * this.scale);
